...
const FacultyDashboard = () => {
  ...
  const handleRenew = async (issueId) => {
    if (!window.confirm('Renew this book for another 30 days?')) return;
    try {
      await api.put(`/issues/${issueId}/renew`);
    ...
    <PageLayout>
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">Welcome back, {user?.name} — browse books or manage your issues.</p>
        </div>
                ...
                <div key={issue._id} className="issue-card">
                  ...
                  {(issue.status === 'Issued' || issue.status === 'Overdue') && (
                    <div className="flex-row" style={{ marginTop: '12px' }}>
                      <button onClick={() => handleRenew(issue._id)} className="btn btn-outline-warning btn-sm">
                        Renew
                      </button>
